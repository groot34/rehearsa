...
- `apps/api/src/routes/tests/kitsRoutes.test.ts`: 47 tests — POST save (auth, 201, 400 invalid/missing, no internal fields), GET list (auth, empty, per-user isolation, summary shape, sort), GET :id (auth, owned, 404 non-owned/non-existent/malformed, no userId), PUT :id (auth, update owned, 404 non-owned, 400 invalid, ownership not changeable), DELETE :id (auth, owned+verify gone, 404 non-owned+original intact, 404 non-existent/malformed), edit survival cycle, multiple kits per user, question confidence tracking (5 tests), question reordering (5 tests), **M10 state restoration (7 tests — fresh kit no M10 fields, each field returned after set, all three together with Appendix A isolation check, ownership isolation for M10 data, M10 absent from list summaries)**.
...
| 2026-09-24 | M10 state restoration | `npm test` | Passed | **246/246 tests passing** across 19 test files (Exit Code 0) — 7 new tests for GET /api/kits/:id M10 state |
