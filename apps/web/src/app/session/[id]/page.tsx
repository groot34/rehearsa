  ...
  const handleSaveKit = async () => {
    ...
    if (res.success && res.data) {
      ...
      if (questionOrder.length > 0) {
        ...
        // Note: This requires the kit service reorder endpoint, which we'll call via the saved kit flow
        // For now, we redirect to the saved kit view
      }
      if (kitRevisionRef.current !== saveRevision) {
        setSaveError('The kit changed while saving. Save again to persist the latest edits.');
      } else {
        // Redirect to the saved kit view
        router.push('/');
      }
    } else {
